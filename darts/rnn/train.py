...
import logging
import argparse
from darts.rnn import genotypes
import torch.nn.functional as F
import torch.backends.cudnn as cudnn
from darts.rnn import data
from darts.rnn import model
from darts.rnn.genotypes import  Genotype
from torch.autograd import Variable
from darts.rnn.utils import batchify, get_batch, repackage_hidden, create_exp_dir, save_checkpoint
...
class Train():
    ...
    def main(self,epoch, arch):
        ...
        try:
            ...
            epoch = 1
            while epoch in tqdm(range(args.epochs + 1)):
                epoch_start_time = time.time()
                try:
                    self.train(corpus, train_data, model, parallel_model, optimizer, epoch)
                except:
                ...
                if 't0' in optimizer.param_groups[0]:
                    ...
                    val_loss2 =  self.evaluate(val_data, corpus, model, parallel_model)
                    logging.info('-' * 89)
                    ...
                    val_loss = self.evaluate(val_data, corpus, model, parallel_model, eval_batch_size)
                    logging.info('-' * 89)
                    logging.info('| end of epoch {:3d} | time: {:5.2f}s | valid loss {:5.2f} | '
                                 'valid ppl {:8.2f}'.format(epoch, (time.time() - epoch_start_time),
        ...
        # Run on test data.
        test_loss = self.evaluate(test_data, corpus, model, parallel_model, test_batch_size)
        logging.info('=' * 89)
    ...
    def evaluate(self,data_source, corpus, model, parallel_model, batch_size=10):
    ...
    def train(self,corpus, train_data, model, parallel_model, optimizer, epoch):
